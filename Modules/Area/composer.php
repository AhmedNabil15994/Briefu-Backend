<?php

view()->composer(['area::dashboard.cities.*','report::dashboard.users.index'], \Modules\Area\ViewComposers\Dashboard\CountryComposer::class);

view()->composer([
    'area::dashboard.states.*','company::dashboard.companies.*'
], \Modules\Area\ViewComposers\Dashboard\CityComposer::class);

view()->composer(['user::dashboard.users.*'],
\Modules\Area\ViewComposers\Dashboard\StateComposer::class);
