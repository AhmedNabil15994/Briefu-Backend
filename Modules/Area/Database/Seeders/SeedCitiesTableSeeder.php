...
class SeedCitiesTableSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        foreach ($this->records as $record) {
            $city = City::create(['status' => 1, 'country_id' => 1]);

            $city->translateOrNew('en')->title = $record['en'];
            $city->translateOrNew('ar')->title = $record['ar'];
            $city->translateOrNew('en')->slug = slugfy($record['en']);
            $city->translateOrNew('ar')->slug = slugfy($record['ar']);
            $city->save();
