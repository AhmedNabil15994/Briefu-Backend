    ...
    */
    ...
    public function __construct()
    {
        ...
        if (env('UPAY_MODE') == 'live_mode') {
            ...
            $this->apiKey = password_hash( env('UPAY_API_KEY') ?? self::API_KEY, PASSWORD_BCRYPT);
        } else {
    ...
    public function send($transaction, $payment, $userToken = '')
    {
        ...
        $extraMerchantsData['ibans'][0] = env('UPAY_IBAN') ?? '';
...
//            'notifyURL' => url(route('client.installments.webhooks')),
        ...
        \File::append(storage_path().'/logs/upay-'.date('Y-m-d').'.log', json_encode($server_output). PHP_EOL);
        \File::append(storage_path().'/logs/upay-fields-'.date('Y-m-d').'.log', json_encode($fields). PHP_EOL);
        \File::append(storage_path().'/logs/upay-transaction-'.date('Y-m-d').'.log', json_encode($transaction). PHP_EOL);
    ...
    public function paymentUrls($type)
    {
        switch ($type){
            case 'subscription':
                $url['success'] = url(route('api.subscriptions.success'));
                $url['failed'] = url(route('api.subscriptions.failed'));
                return $url;
