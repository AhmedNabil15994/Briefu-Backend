        ...
        if (!is_null($index)){
            ...
            if ($setting->get($key)) {
                ...
                foreach($indexes as $index){
                    $arrayOfValues = $value ?? $setting->get($key);
                    $value = array_key_exists($index,$arrayOfValues) ? $arrayOfValues[$index] : null;
                }
...
if (! function_exists('checkUserAccessSubscription')) {

    function checkUserAccessSubscription($user,$key){
        if(!$user->activeSubscription){

            return __('subscription::api.message.must_subscribe_to_subscription_first');

        }elseif(!$user->$key){

            return __('subscription::api.message.'.$key);
        }
